...
class TestCrearPago:
    ...
    def test_crear_pago_exitoso(self, client, user_headers, created_pedido):
        response = client.post(
            "/api/v1/pagos/crear",
            json={"pedido_id": created_pedido["id"], "transaction_amount": created_pedido["total"]},
    ...
    def test_crear_pago_sin_auth_returns_401_o_403(self, client, created_pedido):
        """Sin auth -> 401 o 403 segun implementacion."""
        response = client.post(
            "/api/v1/pagos/crear",
    ...
    def test_crear_pago_duplicado_returns_409(self, client, user_headers, created_pedido):
        payload = {"pedido_id": created_pedido["id"], "transaction_amount": created_pedido["total"]}
    ...
    def test_idempotency_key_generado(self, client, user_headers, created_pedido):
        ...
        assert response.status_code == 201
        assert response.json()["idempotency_key"] is not None
...
class TestObtenerPago:
    ...
    def test_obtener_pago_existente(self, client, user_headers, created_pedido):
        ...
        assert response.json()["pedido_id"] == pedido_id

    def test_obtener_pago_sin_pago_returns_404(self, client, user_headers, created_pedido):
        response = client.get(f"/api/v1/pagos/{created_pedido['id']}", headers=user_headers)
        assert response.status_code == 404

    def test_admin_puede_ver_cualquier_pago(self, client, admin_headers, user_headers, created_pedido):
        pedido_id = created_pedido["id"]
        client.post(
            "/api/v1/pagos/crear",
            json={"pedido_id": pedido_id, "transaction_amount": created_pedido["total"]},
...
class TestWebhookMP:
    ...
    def test_webhook_approved_avanza_pedido(self, client, user_headers, created_pedido):
        ...
        # Parchear la ruta correcta donde el servicio importa el SDK
        with patch("app.modules.pago.pago_service.mercadopago.SDK") as mock_sdk:
        ...
        # El webhook puede devolver 200 con ok o con el resultado del procesamiento
