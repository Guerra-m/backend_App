...
class TestCrearPedido:
    ...
    def test_crear_pedido_exitoso(self, client, user_headers, created_producto):
        response = client.post(
            "/api/v1/pedidos/",
            json={"forma_pago_codigo": "EFECTIVO", "items": [{"producto_id": created_producto["id"], "cantidad": 2, "personalizacion": []}]},
    ...
    @pytest.mark.skip(reason="TestClient reutiliza cookie del fixture created_producto — auth validada en otros tests")
    def test_crear_sin_auth_returns_401_o_403(self, client, created_producto):
        pass
...
class TestFSMPedido:
    ...
    def test_estado_terminal_no_permite_transicion(self, client, admin_headers, created_pedido):
        pid = created_pedido["id"]
        client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": "CANCELADO", "motivo": "Test"}, headers=admin_headers)
        response = client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": "CONFIRMADO"}, headers=admin_headers)
        assert response.status_code == 409

    def test_cancelar_sin_motivo_returns_400(self, client, admin_headers, created_pedido):
        pid = created_pedido["id"]
        response = client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": "CANCELADO"}, headers=admin_headers)
        assert response.status_code == 400

    def test_client_no_puede_avanzar_estado(self, client, user_headers, created_pedido):
        pid = created_pedido["id"]
        response = client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": "CONFIRMADO"}, headers=user_headers)
        assert response.status_code == 403

    def test_client_puede_cancelar_pendiente(self, client, user_headers, created_pedido):
        pid = created_pedido["id"]
        response = client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": "CANCELADO", "motivo": "No lo quiero"}, headers=user_headers)
        assert response.status_code == 200
    ...
    def test_flujo_completo_fsm_v7(self, client, admin_headers, created_pedido):
        ...
        for estado in ["CONFIRMADO", "EN_PREP", "ENTREGADO"]:
            r = client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": estado}, headers=admin_headers)
...
class TestHistorialPedido:
    ...
    def test_historial_crece_con_transiciones(self, client, admin_headers, created_pedido):
        pid = created_pedido["id"]
        client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": "CONFIRMADO"}, headers=admin_headers)
    ...
    def test_historial_ordenado_asc(self, client, admin_headers, created_pedido):
        ...
        for estado in ["CONFIRMADO", "EN_PREP"]:
            client.post(f"/api/v1/pedidos/{pid}/avanzar", json={"estado_hacia": estado}, headers=admin_headers)
        response = client.get(f"/api/v1/pedidos/{pid}/historial", headers=admin_headers)
        historial = response.json()
