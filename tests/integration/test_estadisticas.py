...
class TestResumenKPIs:
    ...
    def test_resumen_excluye_pedidos_cancelados(
        self, client, admin_headers, user_headers, created_pedido
    ):
        """EST-01: pedidos CANCELADOS no suman en ventas."""
        ...
        response = client.get("/api/v1/estadisticas/resumen", headers=admin_headers)
        assert response.status_code == 200
        # pedidos_activos no debe contar el cancelado
        data = response.json()
...
class TestPedidosPorEstado:
    ...
    def test_pedidos_por_estado_client_returns_403(self, client, user_headers):
        response = client.get(
            "/api/v1/estadisticas/pedidos-por-estado", headers=user_headers
        )
        assert response.status_code == 403
